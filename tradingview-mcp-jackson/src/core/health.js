...
export async function launch({ port, kill_existing } = {}) {
  ...
  // Windows MSIX / Microsoft Store fallback: find via Get-AppxPackage
  if (!tvPath && platform === 'win32') {
    try {
      const psOut = execSync(
        'powershell.exe -NoProfile -Command "(Get-AppxPackage -Name TradingView.Desktop).InstallLocation"',
        { timeout: 8000 }
      ).toString().trim();
      if (psOut) {
  ...
  // MSIX apps in WindowsApps cannot be spawned directly — use PowerShell Start-Process
  const isMsix = platform === 'win32' && tvPath.includes('WindowsApps');
  let child;
  if (isMsix) {
    child = spawn(
      'powershell.exe',
      ['-NoProfile', '-Command', `Start-Process '${tvPath.replace(/'/g, "''")}' -ArgumentList '--remote-debugging-port=${cdpPort}'`],
